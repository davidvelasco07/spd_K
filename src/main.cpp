...
int main(int argc, char** argv){
    ...
    Kokkos::initialize( argc, argv );
    {
        ...
        size_t p  = 3;   //Order of space approximation
        size_t N  = 16;
        ...
        double boxlen_x = LENGHT;
        double boxlen_y = LENGHT;
        double boxlen_z = LENGHT;
        ...
        dimension X_dim(_x_,NX,Nx,X*p,comm.x*Nx,boxlen_x,x_fp,X);
        dimension Y_dim(_y_,NY,Ny,Y*p,comm.y*Ny,boxlen_y,x_fp,Y);
        dimension Z_dim(_z_,NZ,Nz,Z*p,comm.z*Nz,boxlen_z,x_fp,Z);
        ...
        #ifdef HYDRO
        double nu=0.00001;
        Hydro_ader system(comm,p,X_dim,Y_dim,Z_dim,x,w,x_sp,x_fp,nu,-2./3*nu);
